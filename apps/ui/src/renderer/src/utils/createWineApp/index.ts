...
export const createWineApp = async (appName: string, config?: WineAppConfig) => {
  ...
  const readAppConfig = async (): Promise<WineAppConfig> => {
    ...
    if (await fileExists(path)) {
      appConfig = JSON.parse(await readFileAsString(path)) as WineAppConfig;
      return appConfig;
    } else {
      return appConfig;
    }
   ...
   */
  const runExe = (args: { path: string; flags?: string }, processArgs?: SpawnProcessArgs) => {
    ...
    return spawnScript('wine', cmd, processArgs);
  };
